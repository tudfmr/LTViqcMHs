...
%S. Gupta and K. Deep / Expert Systems With Applications 119 (2019) 210–230 
...
for i=1:size(X,1)
    [Objective_values(1,i),f(i),g(:,i)]=feval(fobj,X(i,:)',1e20);
    if i==1
...
while t<=Max_iteration
    ...
    if rand<0.1  %%% jr=1
        for i=1:size(X,1)
            ...
            [fnew(1,i),f(i),g(:,i)]=feval(fobj,Xnew(i,:)',Objective_values(1,i));
        ...
        fall=[fnew,Objective_values];
        Xall=[Xnew;X];
        [~,nsort]=sort(fall);
        Objective_values=fall(1,nsort(1:N));
        X=Xall(nsort(1:N),:);
