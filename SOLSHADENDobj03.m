...
for i=1:nsol
    [fitness(i,1),f0(i),g0(:,i)]= feval(fun,pop(i,:)',1e20);
end
...
%     all_res (1:pop_size) = min(fitness);
    ...
    while ~stop_con
        ...
        if rand<PND && nfes<ND_FES 
            ...
            children_fitness=[];
            for i=1:size(ui,1)
                children_fitness(i,1) = feval(fun, ui(i,:)',fitness(i));
            end
...
valParents(loc1) = feval(fun,popold(loc1,:)',valParents(loc1));
nfes=nfes+1;
%% just for printing -- uncomment if it is  required
% for i=1:1    
